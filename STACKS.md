![Stacks logo] (http://catchenlab.life.illinois.edu/stacks/images/stacks_logo.png)

## Tutorial: genotyping a genetic mapping cross

1. [**Preparing the environment, cleaning the data for Stacks**](###Preparing the environment, cleaning the data for Stacks)
2. [**Running the pipeline**]()
3. [**Exporting the results**]()
4. [**Exporting data without use of the database**]()

For this tutorial, we will be working in a directory `~/tutorial/`, located in the home directory. We also assume that Stacks is installed in its default location, with binaries in `/usr/local/bin` and the support files installed in `/usr/local/share/stacks`. We also assume that /usr/local/bin is on the [$PATH.](https://en.wikipedia.org/wiki/PATH_%28variable%29)
